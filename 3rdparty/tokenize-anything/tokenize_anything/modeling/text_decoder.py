...
try:
    ...
    # flash_attn is an optional heavy dependency. These pure-PyTorch fallbacks
    # are mathematically equivalent for the shapes/settings used by this text
    # decoder (interleaved rotary embedding + causal attention with a KV cache).
    import torch.nn.functional as _F

    def apply_rotary_emb(x, cos, sin, interleaved=False, inplace=False, **kwargs):
        # x: (batch, seqlen, nheads, headdim); cos/sin: (seqlen, rotary_dim//2)
        if interleaved:
            cos = cos.repeat_interleave(2, dim=-1)  # (seqlen, rotary_dim)
            sin = sin.repeat_interleave(2, dim=-1)
        else:
            cos = cos.repeat(1, 2)  # (seqlen, rotary_dim), [real, imag] layout
            sin = sin.repeat(1, 2)
        cos = cos.unsqueeze(0).unsqueeze(2)  # (1, seqlen, 1, rotary_dim)
        sin = sin.unsqueeze(0).unsqueeze(2)
        x1 = x * cos
        x2 = x * sin
        if interleaved:
            # pairs are (2i, 2i+1): rotate the "imaginary" part in place
            x2 = torch.stack([-x2[..., 1::2], x2[..., ::2]], dim=-1).flatten(-2)
        else:
            d = cos.shape[-1] // 2
            x2 = torch.cat([-x2[..., d:], x2[..., :d]], dim=-1)
        return x1 + x2

    def flash_attn_func(q, k, v, softmax_scale=None, causal=False, dropout_p=0.0, **kwargs):
        # q, k, v: (batch, seqlen, nheads, headdim)
        return _F.scaled_dot_product_attention(
            q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2),
            scale=softmax_scale, dropout_p=dropout_p, is_causal=causal,
        ).transpose(1, 2)

    def flash_attn_with_kvcache(q, cache_k, cache_v, k, v, softmax_scale=None,
                                causal=True, cache_seqlens=None, **kwargs):
        # q, k, v: (batch, seqlen, nheads, headdim)
        # cache_k/v: (batch, max_seqlen, nheads, headdim)
        # cache_seqlens: (batch,) int32 -- cache length before this step
        batch, seqlen_kv = k.shape[0], k.shape[1]
        for bi in range(batch):
            s = int(cache_seqlens[bi])
            cache_k[bi, s:s + seqlen_kv] = k[bi]
            cache_v[bi, s:s + seqlen_kv] = v[bi]
        total_len = int(cache_seqlens.max()) + seqlen_kv
        K = cache_k[:batch, :total_len]  # (batch, total_len, nheads, headdim)
        V = cache_v[:batch, :total_len]
        q_len = q.shape[1]
        q_pos = torch.arange(q_len, device=q.device).unsqueeze(0) + cache_seqlens.unsqueeze(1)  # (batch, q_len)
        k_pos = torch.arange(total_len, device=q.device).unsqueeze(0)  # (1, total_len)
        mask = q_pos.unsqueeze(2) >= k_pos.unsqueeze(1)  # (batch, q_len, total_len)
        q_t = q.transpose(1, 2).float()  # (batch, nheads, q_len, headdim)
        K_t = K.transpose(1, 2).transpose(-1, -2).float()  # (batch, nheads, headdim, total_len)
        scores = (q_t @ K_t) * softmax_scale  # (batch, nheads, q_len, total_len)
        scores = scores.masked_fill(~mask.unsqueeze(1), float("-inf"))
        attn = torch.softmax(scores, dim=-1)
        return (attn @ V.transpose(1, 2).float()).transpose(1, 2).to(q.dtype)
