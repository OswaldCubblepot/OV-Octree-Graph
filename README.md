...
The class-name text features used for evaluation are not shipped in the repository. Generate them with (requires the `clip` / `open_clip_torch` packages and downloads the CLIP ViT-L/14 / ViT-H-14 weights):

```bash
python scripts/generate_vocab_features.py --models l14vild,h14
```

This writes `replica101_*.npy`, `scannet20_*.npy` and `scannet200_*.npy` under `ovgraph/evaluation/voc_features/`. (Alternatively, `ovgraph/evaluation/comput_voc_feature.py` reproduces the official features through the OVSeg model.)

### 2.5 Pretrained Checkpoints
...
### 6. Octree-Graph and Downstream Tasks

After the build, every scene owns an adaptive octree-graph (`*-octree_graph.pkl`), in which each node is an instance with its semantics, center and an adaptive occupancy octree, and each edge encodes the spatial relation ("left of", "above", ...), distance and 3D offset between instances.
...
# text-based instance retrieval
python scripts/octree_demo.py --config configs/replica/replica_cropformer_ovseg_tap.yaml \
    --scene room0 --task retrieve --query "a red chair" --top_k 5

# occupancy query: which instance occupies a 3D point
python scripts/octree_demo.py --config configs/replica/replica_cropformer_ovseg_tap.yaml \
    --scene room0 --task occupancy --point=1.0,0.5,1.2

# path planning (A* on the free voxels of the adaptive octrees)
