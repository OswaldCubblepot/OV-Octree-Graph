...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        text_feats = np.concatenate(text_feats, axis=0)
        # save
        np.save(f"ovgraph/evaluation/voc_features/scannet200_clip_h_14.npy", text_feats)
        ...
        text_feats = np.concatenate(text_feats, axis=0)
        # save
        np.save(f"ovgraph/evaluation/voc_features/scannet20_clip_h_14.npy", text_feats)
        ...
        # save
        np.save(f"ovgraph/evaluation/voc_features/replica101_clip_h_14.npy", text_feats)
        ...
        np.save(f"ovgraph/evaluation/voc_features/scannet200_clip_b_32.npy", vocab_features)

        # #* scanenet *# #
        vocab_features = get_clip_feature(clip_model=clip_model, text_label=CLASS_LABELS_20, normalize=True, device=f"cuda:{0}")
        np.save(f"ovgraph/evaluation/voc_features/scannet20_clip_b_32.npy", vocab_features)

        # * replica * #
        vocab_features = get_clip_feature(clip_model=clip_model, text_label=REPLICA_CLASSES, normalize=True, device=f"cuda:{0}")
        np.save(f"ovgraph/evaluation/voc_features/replica101_clip_b_32.npy", vocab_features)

        # 2. multiple prompts
        # #* scanenet *# #
        vocab_features = extractor(CLASS_LABELS_200, clip_model).cpu().numpy()
        np.save(f"ovgraph/evaluation/voc_features/scannet200_clip_b_32_vild.npy", vocab_features)

        # #* scanenet *# #
        vocab_features = extractor(CLASS_LABELS_20, clip_model).cpu().numpy()
        np.save(f"ovgraph/evaluation/voc_features/scannet20_clip_b_32_vild.npy", vocab_features)

        # * replica * #
        vocab_features = extractor(REPLICA_CLASSES, clip_model).cpu().numpy()
        np.save(f"ovgraph/evaluation/voc_features/replica101_clip_b_32_vild.npy", vocab_features)
        ...
        np.save(f"ovgraph/evaluation/voc_features/scannet200_clip_l_14_vild.npy", class_feats.cpu().numpy())

        # * scanenet * #
        class_feats = demo.predictor.model.clip_adapter.get_text_features(CLASS_LABELS_20)
        np.save(f"ovgraph/evaluation/voc_features/scannet20_clip_l_14_vild.npy", class_feats.cpu().numpy())

        # * replica * #
        class_feats = demo.predictor.model.clip_adapter.get_text_features(REPLICA_CLASSES)
        np.save(f"ovgraph/evaluation/voc_features/replica101_clip_l_14_vild.npy", class_feats.cpu().numpy())
