...
class OVsegExtractor:
    def __init__(
        ...
        # CLIP_MODEL_NAME points at the ViT-L/14 state_dict extracted from the
        # OVSeg checkpoint (openaipublic.azureedge.net is unreachable here)
        self.demo = VisualizationDemo(setup_cfg(ovseg_config, [f"MODEL.WEIGHTS", "pretrained_weights/ovseg_swinbase_vitL14_ft_mpt.pth",
                                                               f"MODEL.CLIP_ADAPTER.CLIP_MODEL_NAME", "pretrained_weights/openai_vitl14_from_ovseg.pt"]))
...
class OVsegTapExtractor:
    def __init__(
        ...
        # 3. (skipped) open_clip ViT-H-14 is dead code for this config: it is
        #    only used in forward_region's `back_feat == 'clip'` branch, and the
        #    Replica config sets back_feat=ovseg. Loading it would add a ~2.5 GB
        #    download + ~2.5 GB VRAM that the 8 GB GPU cannot spare, with no
