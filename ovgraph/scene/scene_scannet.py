...
class ScanNetSceneGraph:
    ...
    def evaluate_retrieval(self):
        ...
        # class feature
        # ----------------------------------------------------------------------------------------- #
        vocab_features = np.load("./ovgraph/evaluation/voc_features/scannet200_clip_l_14_vild.npy")
        ...
        pred_features, pred_masks, pred_caption_features, pred_features_list, _ = get_predicted_instances(scene_graph, self.cfg.eval.feature_name)
        ...
        output_file = f"proposed_fusion_{self.out_file_prefix}_interval-{self.cfg.merge.interval}-{self.cfg.eval.feature_name}-retrieval.pkl"
    ...
    def evaluate_segmentation_20_opt(self):
        ...
        for idx, node in enumerate(scene_graph.nodes):
            scene_graph.nodes[node]['top1_cls'] = CLASS_LABELS_20[pred_class[idx]]
    ...
    def build_octree_graph(self):
        ...
        if not os.path.isfile(scene_graph_path):
            ...
            print(f"no scene graph found in {self.output_predict_folder}, "
                  f"run obtain_3d_segment / evaluate_segmentation_20_opt first")
            print("========================================")
            return None
