...
class ReplicaSceneGraph:
    def __init__(
        ...
        # The Replica mesh PLY uses a custom face layout (vertex_indices + object_id)
        # that open3d's RPly parser rejects ("RPly: Invalid file format"), silently
        # returning an empty point cloud. Read the vertices with plyfile instead
        # (same approach as read_ply_and_assign_colors_replica below).
        _gt_ply = plyfile.PlyData.read(gt_ply_path)
        _gt_vertices = np.vstack([_gt_ply["vertex"]["x"], _gt_ply["vertex"]["y"], _gt_ply["vertex"]["z"]]).T
        self.gt_point = o3d.geometry.PointCloud()
        self.gt_point.points = o3d.utility.Vector3dVector(_gt_vertices)
    ...
    def generate_2d_proposals(self):
        ...
        # free the detector; the downstream stages rebuild their own models and
        # the 8 GB GPU cannot keep both the detector and the TAP extractor resident
        del self.detector
    ...
    def compute_2d_feature(self):
        ...
        # free the extractor; graph_back_projcection rebuilds its own OVSeg+TAP
        # extractor and the 8 GB GPU cannot keep two of them resident at once
        del self.extractor
        torch.cuda.empty_cache()
    ...
    def evaluate_segmentation_opt(self):
        ...
        _pred_features = rep_dif_denoise(pred_features_list)
        pred_features = pred_features if _pred_features is None else _pred_features
    ...
    def build_octree_graph(self):
        """Build the adaptive octree-graph from the finished scene graph."""
        from ..structure.octree_graph import OctreeGraph

        # prefer the graph with assigned classes, fall back to the raw one
        for suffix in ["-backprj-top1.pkl", "-backprj.pkl"]:
            output_file = f"proposed_fusion_{self.out_file_prefix}_{self.cfg.extractor.back_feat}_interval-{self.cfg.merge.interval}{suffix}"
            scene_graph_path = os.path.join(self.output_predict_folder, output_file)
            if os.path.isfile(scene_graph_path):
                break

        if not os.path.isfile(scene_graph_path):
            print("========================================")
            print(f"no scene graph found in {self.output_predict_folder}, "
                  f"run obtain_3d_segment / evaluate_segmentation_opt first")
            print("========================================")
            return None
        ...
        octree_graph = OctreeGraph.build(
            scene_graph,
            self.gt_xyz,
            min_cell_size=self.cfg.octree.min_cell_size,
            max_depth=self.cfg.octree.max_depth,
            edge_dist=self.cfg.octree.edge_dist,
        )

        output_file = f"proposed_fusion_{self.out_file_prefix}_interval-{self.cfg.merge.interval}-octree_graph.pkl"
        output_path = os.path.join(self.output_predict_folder, output_file)
        octree_graph.save(output_path)
        n_cells = sum(len(a['octree']) for _, a in octree_graph.nodes(data=True))
        print(f"[octree-graph] {octree_graph.number_of_nodes()} nodes, "
              f"{octree_graph.number_of_edges()} edges, {n_cells} occupied cells, "
              f"{octree_graph.storage_bytes() / 1024:.1f} KB; saved to {output_path}")
        return octree_graph
...
def rep_dif_denoise(pred_features_list, neighbor_rate: float = 0.3):
    # when feature_name == "feature" each node owns a single feature and
    # there is nothing to denoise; the caller keeps the mean features
    if any(cluster is None for cluster in pred_features_list):
        return None
